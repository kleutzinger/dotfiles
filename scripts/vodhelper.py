...
def init_tourney_yaml() -> dict:
    ...
    try:
        bracket = list(filter(lambda x: x.get("Date") == nTime, brackets))[0]
    except IndexError:
        bracket = None
        guess_trny_url = None
        title = "?"
...
def gen_perspective_ffmpeg_cmd(
    ...
    cmd = (
        ...
        f"""drawtext=fontfile='{font_path}':text='{text_overlay}':fontcolor=white:fontsize=20:box=1:boxcolor=black@0.5:boxborderw=5:x=10:y=10 """
        "-s 960x720 "
        f"'{o_vid_path}'"
    )
